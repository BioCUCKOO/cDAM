...
class Dsf_model(nn.Module):
    def __init__(self, dim_in, dim_out):
        super(Dsf_model, self).__init__()
...
with open('Data/teacher_model_positive_gene.txt', 'r')as f:
    ...
    for line in readlines:
        gene = line.strip('\n')
        dict_at1[gene] = 'yes'
with open('Data/Dsf_model_positive_gene.txt', 'r')as f:
    dict_at2 = {}
    readlines = f.readlines()
    for line in readlines:
        gene = line.strip('\n')
...
def plot_roc_curve(fper, tper,auc):
    ...
    #plt.savefig(Dsf_model_auc.png)
    plt.close()

if __name__ == '__main__':
    model1 = load_checkpoint('Model/Dsf_model')
    ...
    print('Dsf_model_auc:', auc)

    pre_label = X_label.tolist()
    pre_value = y_pred.tolist()
    with open('Data/Dsf_model_predictor_result.txt', 'w')as f:
