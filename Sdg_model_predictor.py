...
class SDG_model(nn.Module):
    def __init__(self, dim_in, dim_out):
        super(SDG_model, self).__init__()
    ...
    def forward(self, X):
        ...
        return output
with open('Data/SDG_teacher.txt', 'r')as f:
    dict_at1 = {}
    readlines = f.readlines()
    for line in readlines:
        gene = line.strip('\n')
        dict_at1[gene] = 'yes'
with open('Data/SDG_posi.txt', 'r')as f:
    dict_at2 = {}
    readlines = f.readlines()
    for line in readlines:
        gene = line.strip('\n')
        dict_at2[gene] = 'yes'
with open('Data/SDG_ppi.txt', 'r')as f:
    dict_PPI = {}
    readlines = f.readlines()
    for line in readlines:
        gene = line.strip('\n').split('\t')[0]
...
def load_checkpoint(path: str):
    ...
    return model
def prepare_feature():
    with open('Data/SDG_omics_feature.txt','r')as f:
        one_feature = []
        name_list = []
        readlines = f.readlines()
        for line in readlines:
...
if __name__ == '__main__':
    model1 = load_checkpoint('Model/Sdg_model')
    ...
    print('Sdg_model_auc:', auc)

    pre_label = X_label.tolist()
    pre_value = y_pred.tolist()
    with open('Data/SDG_model_predictor_result.txt', 'w')as f:
